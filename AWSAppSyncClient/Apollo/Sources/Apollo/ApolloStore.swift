...
public final class ApolloStore {
  ...
  func publish(records: RecordSet, context: UnsafeMutableRawPointer? = nil) -> Promise<Void> {
    ...
    func publishWithLatest(records: RecordSet, context: UnsafeMutableRawPointer? = nil) -> Promise<[Record]> {
        return Promise<[Record]> { fulfill, reject in
            queue.async(flags: .barrier) {
                self.cacheLock.withWriteLock {
                    self.cache.mergeWithLatest(records: records)
                    }?.andThen { (changedKeys, recordAfter) in
                        self.didChangeKeys(changedKeys, context: context)
                        fulfill(recordAfter)
                }
            }
