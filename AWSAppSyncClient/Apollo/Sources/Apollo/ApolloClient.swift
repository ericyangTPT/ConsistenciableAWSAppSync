...
public class ApolloClient {
    ...
    @discardableResult public func fetchWithConsistencyCheck<Query: GraphQLQuery>(query: Query, cachePolicy: CachePolicy = .returnCacheDataElseFetch, queue: DispatchQueue = DispatchQueue.main, resultHandler: OperationResultHandler<Query>? = nil) -> Cancellable {
        return _fetchWithConsistencyCheck(query: query, cachePolicy: cachePolicy, queue: queue, resultHandler: resultHandler)
    }
    ...
    func _fetchWithConsistencyCheck<Query: GraphQLQuery>(query: Query, cachePolicy: CachePolicy, context: UnsafeMutableRawPointer? = nil, queue: DispatchQueue, resultHandler: OperationResultHandler<Query>?) -> Cancellable {
        // If we don't have to go through the cache, there is no need to create an operation
        // and we can return a network task directly
        return sendWithFetchCache(query: query, cachePolicy: cachePolicy, queue: queue, resultHandler: resultHandler)
    }
    ...
    fileprivate func sendWithFetchCache<Query: GraphQLQuery>(query: Query,
        ...
        return networkTransport.send(operation: query) { (response, error) in
            ...
            firstly {
                try response.parseResult(cacheKeyForObject: self.store.cacheKeyForObject)
                    ...
                    if let records = records {
                        firstly{
                            self.store.publishWithLatest(records: records, context: context).catch { error in
                                preconditionFailure(String(describing: error))
                            }
                            }.andThen {recordAfter in
                                self.store.load(query: query) { (resultAfter, error) in
                                    guard let resultHandler = resultHandler else { return }
                                    
                                    resultHandler(resultAfter, error)
                                }
                        }
                    }else {
