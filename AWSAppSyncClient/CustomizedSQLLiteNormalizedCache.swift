//
//  CustomisedSQLLiteNormalizedCache.swift
//  AWSAppSync
//
...
public final class CustomizedSQLLiteNormalizedCache: NormalizedCache {
    ...
    public func mergeWithLatest(records: RecordSet) -> Promise<RecordsResult>? {
        return Promise { try mergeRecordsWithLatest(records: records) }
    }
    ...
    private func mergeRecords(records: RecordSet) throws -> Set<CacheKey> {
        ...
        let recordsResult = recordSet.mergeWithLatest(records: records)
        let changedFieldKeys = recordsResult.0
    ...
    private func mergeRecordsWithLatest(records: RecordSet) throws -> RecordsResult {
        var recordSet = RecordSet(records: try selectRecords(forKeys: records.keys))
        let recordsResult = recordSet.mergeWithLatest(records: records)
        let changedFieldKeys = recordsResult.0
        let recordsAfterMerge = recordsResult.1
        let changedRecordKeys = changedFieldKeys.map { recordCacheKey(forFieldCacheKey: $0) }
        for recordKey in Set(changedRecordKeys) {
            if let recordFields = recordSet[recordKey]?.fields {
                let recordData = try SQLiteSerialization.serialize(fields: recordFields)
        ...
        return RecordsResult(Set(changedFieldKeys), recordsAfterMerge)
    }
